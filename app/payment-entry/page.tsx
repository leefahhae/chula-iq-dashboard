...
export default function PaymentEntryPage() {
  const { students, courses, enrollments, addTransaction, getCourse } = useStore();
  ...
  const suggestedAmount = React.useMemo(() => {
    ...
    if (selectedCourse.type === "private") {
      const rate = selectedCourse.hourlyRate ?? 0;
      return Math.round(selectedEnrollment.hoursUsed * rate);
    }
    return selectedEnrollment.monthlyFee ?? selectedCourse.monthlyFee ?? 0;
